...
- [6. Maintenance and Operational Tasks](#6-maintenance-and-operational-tasks)
  ...
  - [6.12 Installing a New TLS Certificate](#612-installing-a-new-tls-certificate)
...
The certificate and key must be named `tls.crt` and `tls.key` as in the example above.

⚠️ NOTE: A self-signed certificate can be generated by enabling the `ssl_self_signed` option in the configuration file.

To **replace an existing certificate** and apply it without downtime, see [6.12 Installing a New TLS Certificate](#612-installing-a-new-tls-certificate)
...
### 6.12 Installing a New TLS Certificate
  
If you are **replacing an old certificate**, you can apply the certificate without running a full [application deployment](#4-application-deployment).
